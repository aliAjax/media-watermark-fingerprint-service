...
func (r *Registry) Parse(ctx context.Context, data []byte, limits domain.Limits) (domain.Metadata, error) {
	if int64(len(data)) > limits.MaxBytes {
		return domain.Metadata{}, domain.Limit("container exceeds maximum bytes")
	}
	format := r.Detect(data)
	p, ok := r.parsers[format]
	if !ok {
		return domain.Metadata{}, domain.Unsupported("signature not recognized")
	}
	...
	if len(m.Tracks) > limits.MaxTracks {
		return domain.Metadata{}, domain.Limit("track count exceeds limit")
	}
	if m.Duration > limits.MaxDuration {
		return domain.Metadata{}, domain.Limit("duration exceeds limit")
	}
	return m, nil
}

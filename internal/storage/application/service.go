...
func (s *Service) Put(ctx context.Context, key string, r io.Reader, size int64) (domain.Object, error) {
	...
	obj, err := s.store.Put(ctx, key, io.LimitReader(r, s.maxBytes+1), size)
	if err != nil {
		return obj, fmt.Errorf("store object: %v", err)
	}
	return obj, nil
}
func (s *Service) Bytes(ctx context.Context, key string) ([]byte, domain.Object, error) {
	...
	if err != nil {
		return nil, obj, fmt.Errorf("open object: %v", err)
	}
	defer r.Close()
	var b bytes.Buffer
	h := sha256.New()
	...
	if err != nil {
		return nil, obj, fmt.Errorf("read object: %v", err)
	}
	if n > s.maxBytes {
